...
const RunewordList = ({ dataType }: { dataType: DataType }) => {
  ...
  const runewords = Object.keys(content.content.runewordsTable)
    ...
    .sort((a, b) => {
      // Remove newlines (\n) or special characters before comparing
  ...
  const filteredByRunes =
    selectedRunes.length > 0
      ? filteredBySearch.filter((runeword) => {
  ...
  return (
    <Container>
      ...
      <StyledText>
        <StyledKeyword $color={theme.color.unique}>Runewords</StyledKeyword> are
        combinations of several different{" "}
        <StyledLink to={toRunes()}>runes</StyledLink> designed to create
        powerful weapons or armor. The correct arrangement of runes in socketed
        items not only provides basic attributes but also bonus effects that
        enhance the item's power. Most of the most important weapons and armor
        in the game come from runewords. Many of these are very difficult to
        obtain due to the rare appearance of runes at higher levels. Besides the
        runes, you also need to find the appropriate item. It is worth
        mentioning that runewords that deal high damage are best combined with
        items that also have a high base attack rating. Experienced players
        spend a lot of time searching for the right items and runes to
        ultimately create their desired equipment.
      </StyledText>
      ...
      <FilterContainer $isOpen={isOpen}>
        ...
        />
        <FilterTitle>Filter by runes</FilterTitle>
        <RuneGridContainer>
          ...
          <StyledTable>
            ...
            <tbody>
              {filteredByRunes.map((runeword, index) => {
                const tableKey =
                  Object.keys(content.content.runewordsTable).find(
                    (key) =>
                      content.content.runewordsTable[key][0] === runeword[0],
                  ) || "";

                const isNew = tableKey.includes("NEW");
